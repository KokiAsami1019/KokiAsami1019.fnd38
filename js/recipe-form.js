...
function addIngredient() {
    ...
    unitSelect.innerHTML = `
        ...
        <option value="pieces1">個</option>
        <option value="pieces2">片</option>
        <option value="pieces3">粒</option>
        <option value="pieces4">尾</option>
        <option value="pieces5">房</option>
        <option value="pieces6">パック</option>
        <option value="numbers1">本</option>
        <option value="numbers2">枚</option>
    `;
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    if (recipeId !== null) {
        ...
        populateForm(recipe, htmlRecipe); 
    }
});
...
function populateForm(recipe, htmlRecipe) {
    ...
    htmlRecipe.ingredients.forEach(ingredient => {
        ...
        unitSelect.innerHTML = `
            ...
            <option value="pieces1" ${ingredient.unit === 'pieces1' ? 'selected' : ''}>個</option>
            <option value="pieces2" ${ingredient.unit === 'pieces2' ? 'selected' : ''}>片</option>
            <option value="pieces3" ${ingredient.unit === 'pieces3' ? 'selected' : ''}>粒</option>
            <option value="pieces4" ${ingredient.unit === 'pieces4' ? 'selected' : ''}>尾</option>
            <option value="pieces5" ${ingredient.unit === 'pieces5' ? 'selected' : ''}>房</option>
            <option value="pieces6" ${ingredient.unit === 'pieces6' ? 'selected' : ''}>パック</option>
            <option value="numbers1" ${ingredient.unit === 'numbers1' ? 'selected' : ''}>本</option>
            <option value="numbers2" ${ingredient.unit === 'numbers2' ? 'selected' : ''}>枚</option>
        `;
        const amountInput = document.createElement('input');
        amountInput.type = 'text';
        amountInput.name = 'ingredient-amount';
